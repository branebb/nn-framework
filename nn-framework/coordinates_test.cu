	...
	batch_size(batch_size), number_of_batches(number_of_batches)
{
	for (int i = 0; i < number_of_batches; i++) {
		...
		targets.push_back(Matrix(Dimensions(batch_size, 1)));
		...
		for (int k = 0; k < batch_size; k++) {
			...
			batches[i][batches[i].dims.x + k] = static_cast<float>(rand()) / RAND_MAX - 0.5;;

			if ( (batches[i][k] > 0 && batches[i][batches[i].dims.x + k] > 0) ||
				 ((batches[i][k] < 0 && batches[i][batches[i].dims.x + k] < 0)) ) {
				targets[i][k] = 1;
			}
			else {
				targets[i][k] = 0;
